...
const SubscriptionModal = ({user}: Props) => {
    ...
    console.log("Payment is successful:", paymentIntent)
    router.refresh()
    } catch (error) {
    ...
    <Dialog>
        ...
        Subscription
          ...
          <DialogFooter className='gap-4 items-center'>
           <DialogClose
           className="w-full sm:auto border border-border rounded-md px-3 py-2" disabled={loading}
           >
            Cancel
           </DialogClose>

           <Button type="submit" className='w-full sm:w-auto' onClick={handleConfirm} disabled={loading} >
             {loading ? (
                <>
