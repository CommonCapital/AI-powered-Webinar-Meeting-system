...
{Array.from({length: 3}).map((_, index) => (
    <Image src="/feature-card.jpeg" alt="feature-card" width={250} height={250} className='w-full h-full object-cover rounded-xl' key={index}
    />
))}
</div>
...
<FeatureSectionLayout heading="See the list off your current customers " link="/pipeline" >
...
{potentialCustomers.slice(0, 2 ).map((customer, index) => (
    <UserInfoCard key={index} customer={customer} tags={customer.tags}/>
   
))}
    <Image src="/feature-card.jpeg" alt="feature-card" width={350} height={350} className='object-cover rounded-xl absolute px-5 mb-28 hidden sm:flex backdrop-blur-[20px]' 
    />
</div>
</FeatureSectionLayout>
        </div>
