...
import { HomeIcon, User2Icon, Webcam } from 'lucide-react'
...
const WebinarPage = async (props: Props) => {
 ...
 <Tabs defaultValue="all" className='w-full flex flex-col gap-8'>
      ...
      rightIcon={<User2Icon className="w-4 h-4" />}
...
{webinars?.length > 0 ? (webinars.map((webinar: Webinar, index: number) => (
    <WebinarCard 
key={index} webinar={webinar}
    />
))
):(
